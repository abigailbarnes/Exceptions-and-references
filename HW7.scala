...
def meld(tau1:Type, tau2:Type): Option[Type] =
  tau1 match 
    case _ if (tau1 == tau2) => Some(tau1) // t1 + t1
    case TyFunc(temptau1, temptau2) => tau2 match
      case TyFunc(temptau3, temptau4) => Some(TyFunc(meld(temptau1, temptau3).get, meld(temptau2, temptau4).get))
      case _ => None
    case Morph if (tau2 != Morph) => Some(tau2)//M + t2
    case TyRef(temptau1)=> tau2 match
      case TyRef(temptau2) => Some(TyRef(meld(temptau1, temptau2).get))
      case _ => None
    case _ => tau2 match
      case Morph => Some(tau1) //t1 + M
      case _ => None //consider making none cases morph
...
def typeOf(gamma:TypeEnv, t:Term): Type = 
  t match
    ...
    case TmApp(t1, t2) =>
      var type1 = typeOf(gamma, t1)
      var type2 = typeOf(gamma,t2)
      var melded = meld(type1, TyFunc(type2, Morph)).get
      melded match 
        case TyFunc(type3, type4) => type4
    ...
    case TmLet(varName, term1, term2) =>
      if(typeOf(gamma, TmVar(varName)) == typeOf(gamma, term1))
      {
        return typeOf(gamma, term2)
      }
      else
      {
        throw new Exception("error typeof: term ill-typed")
      }
      /*typeOf(gamma, TmVar(varName)) match
        //checking to see if the type of the variable is in the type environment and
        //has the same type as term1
        case typeOf(gm, term1) => typeOf(gamma, term2)*/
    case TmIf(t1, t2, t3) => meld(typeOf(gamma, t2), typeOf(gamma,t3)).get
    case TmRaise(t1) => meld(typeOf(gamma, t1), TyExn).get //morph?
    case TmTry(t1, t2) => 
      var type1 = typeOf(gamma, t1)
      var type2 = typeOf(gamma, t2)
      var almosttype3 = meld(type2, TyFunc(TyExn, Morph)).get
      almosttype3 match
        case TyFunc(TyExn, type3) =>
          meld(type1, type3).get
        case _ => throw new Exception("error typeof: term ill-typed")
    case TmEq(t1, t2) => TyBool //check this case?
    case TmRef(t1) => TyRef(typeOf(gamma,t1))
    case TmRead(t1) => 
      var type1 = typeOf(gamma, t1)
      var almosttype2 = meld(type1, TyRef(Morph)).get
      almosttype2 match
        case TyRef(type2) => type2
        case _ => throw new Exception("error typeof: term ill-typed")
    case TmWrite(t1, t2) => TyUnit
      /*var type1 = typeOf(gamma,t1)
      var type2 = typeOf(gamma, t2)
      var almosttype3 = meld(type1, TyRef(Morph))
      almosttype3 match
        case TyRef(type3) =>*/
    case TmSeq(t1) => typeOf(gamma, t1(t1.length - 1))
